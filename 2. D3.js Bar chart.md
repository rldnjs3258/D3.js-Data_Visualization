# 1. 바차트 그리기
## (1) 팁
  - Atom 에디터를 이용하면 Html, Css, JavaScript 등의 파일을 편하게 관리할 수 있다.
  - 완성한 소스코드를 실행한 후, 우클릭 - 검사 - Elements/Console을 통해 소스코드 및 로그를 확인할 수 있다.
  - D3.js에서 translate()은 '전체 그림'의 위치를 움직인다.
  - D3.js를 통해 svg에 그림을 그릴 때, 자동으로 데이터 갯수만큼 그림을 그린다.


  - JavaScript에는 function이라는 함수가 존재한다. 이 함수는 데이터를 반환해 준다.

```javascript
function(d){ // d는 임의로 지정해 준 것으로, 바꿔도 됨!
	return d;
}
```

  - JavaScript에서는 console.log를 이용해서 로그를 찍어 볼 수 있고, 로그를 통해 디버깅에 이용할 수 있다.

```javascript
console.log(d); // d에 얻고 싶은 로그의 변수를 입력하자.
```

  - D3.js에서는 '주로' d로 데이터, i로 인덱스를 정의한다.

```javascript
function(d,i){
	return i * 30; // 인덱스 값에 30씩 곱한다.
}
```
